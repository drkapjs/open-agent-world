...
function FitPlot({ plot }: { plot: NonNullable<PipelineCandidate['plot']> }) {
  ...
  return <div className="xrd-fit-plot"><svg viewBox="0 0 670 230" role="img" aria-label="实验谱与全谱拟合结果"><line x1="40" x2="640" y1="188" y2="188" stroke="currentColor" opacity=".4"/><polyline points={line(observed)} fill="none" stroke="#b9d8dd" strokeWidth="1.2"/><polyline points={line(calculated)} fill="none" stroke="#e99b71" strokeWidth="1.2"/>{Array.from({ length: 6 }, (_, i) => lo + (hi - lo) * i / 5).map(value => <text key={value} x={x(value)} y="210" fill="currentColor" fontSize="11" textAnchor="middle">{value.toFixed(1)}</text>)}<text x="335" y="228" fill="currentColor" fontSize="11" textAnchor="middle">2θ / ° · 实验与计算使用同一强度尺度</text></svg><small>蓝：实验谱　铜：计算谱</small></div>;
}
